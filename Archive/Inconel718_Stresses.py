...
with open(file_path, 'r') as file:
    for line in file:
        ...
        s2 = stress_t2
        von_mises_t = np.sqrt(0.5 * ((s1 - s2)**2 + (s2)**2 + (s1)**2))
        von_mises.append(von_mises_t * 1e-6)
        sf = np.divide(yieldstress, von_mises)
        strain.append(100*von_mises_t / E)
...
ax5 = ax[1,1]
ax5.plot(pos, strain, color="tab:blue")
ax5.set_ylabel("Strain (%)")
ax5.set_xlabel("Axial Distance")
ax5.set_xlim(0, None)
ax5.grid()
ax5.legend(["Strain"])
plt.tight_layout()
